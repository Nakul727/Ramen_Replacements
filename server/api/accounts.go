...
func CreateAccount(c *gin.Context) {
	...
	res, err := utils.PrepareAndExecute(db, "SELECT * FROM Users WHERE name=$1 OR email=$2", acc.Username, acc.Email)
	if err != nil {
		utils.RespondWithError(c, http.StatusInternalServerError, err.Error())
		return
	}
	...
	_, err = utils.PrepareAndExecute(db, "INSERT INTO Users (name, email, pass, pfp) VALUES ($1, $2, $3, $4)", acc.Username, acc.Email, acc.Password, acc.PFP)
	if err != nil {
		utils.RespondWithError(c, http.StatusInternalServerError, err.Error())
		return
	}
