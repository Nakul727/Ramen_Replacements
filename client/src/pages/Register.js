...
function Register() {
  ...
  const register_user = async () => {
    ...
    if (pass === '' || passConfirm === '') {
      ...
      setPassConfirmError(
        'Password is too short. You must choose a password within 3-20 characters.',
      );
      hasErrors = true; // Set the flag to true
    } else if (pass.length > maxPasswordLen) {
      setPassError('Password is too long. You must choose a password within 3-20 characters.');
      setPassConfirmError(
        'Password is too long. You must choose a password within 3-20 characters.',
      );
      hasErrors = true; // Set the flag to true
    } else {
      setPassError('');
    ...
    <div>
      ...
      {isLoggedIn ? (
        <div className="body_sections">
          <div className="mt-40">You are already logged in</div>
        </div>
      ) : (
        <div className="body_sections overflow-hidden pt-20">
          <div
            className="Form py-20 mx-56 mb-12 rounded-3xl"
          ...
          >
            ...
            <div className="px-32 mt-24">
              <label
                htmlFor="name"
                className={`font-arvo text-sm text-gray-600 mb-2 ${usernameError ? 'text-red-500' : ''}`}
              >
                Username*
              </label>
              <input
            ...
            <div className="px-32 mt-4">
              <label
                htmlFor="email"
                className={`font-arvo text-sm text-gray-600 mb-2 ${emailError ? 'text-red-500' : ''}`}
              >
                Email*
              </label>
              <input
            ...
            <div className="px-32 mt-4">
              <label
                htmlFor="pass"
                className={`font-arvo text-sm text-gray-600 mb-2 ${passError ? 'text-red-500' : ''}`}
              >
                Password*
              </label>
              <input
            ...
            <div className="px-32 mt-4">
              <label
                ...
                className={`font-arvo text-sm text-gray-600 mb-2 ${passConfirmError ? 'text-red-500' : ''}`}
              >
                Confirm Password*
              </label>
              <input
            ...
            <div
              className="text-center font-arvo text-red-900 mt-10"
